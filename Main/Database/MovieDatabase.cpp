...
#include <sstream>
#include <iostream>
#include <algorithm> // for trim helper
...
vector<Theater> loadDatabase(const string& filename)
{
    ...
    while (getline(file, line)) {
        ...
        if (line.find("Theater:") == 0) {
            // Save previous movie, hall, theater
...
// Save booked seats (overwrite or add the matching entry)
void saveBookedSeats(const string& theaterName, const string& hallName, const string& movieTitle, const string& showtime, const vector<int>& bookedSeats) {
    ifstream fileIn("../Database/seatBookings.txt");
    vector<string> lines;
    string line;
    bool found = false;
    size_t i = 0;
    ...
    ofstream fileOut("../Database/seatBookings.txt");
